...
const AITutorPanel: React.FC<{ material: StudyMaterial }> = ({ material }) => {
  ...
  useEffect(() => {
    const initializeChat = () => {
        try {
          if (!process.env.API_KEY) throw new Error("A chave de API não foi configurada.");
          ...
          setChat(newChat);
          setHistory([{ role: 'model', parts: [{ text: `Olá! Sou o VandaBot. Vejo que você está estudando ${material.title}. Como posso ajudar?` }] }]);
        } catch (err) {
          console.error("Erro ao inicializar o Tutor IA:", err);
          setError("Desculpe, não foi possível conectar ao Tutor IA.");
        } finally {
            setIsLoading(false);
        }
    }
  ...
  return (
    <div className="bg-slate-800/60 rounded-xl border border-slate-700 p-4 flex flex-col h-[75vh] lg:h-full">
      <h3 className="text-xl font-bold text-cyan-400 mb-4 text-center">Converse com o Tutor</h3>
      ...
      <form onSubmit={handleFormSubmit} className="flex gap-2 flex-shrink-0">
        <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder={error || "Faça uma pergunta..."} disabled={!chat || isLoading || !!error} className="flex-1 bg-slate-700 border border-slate-600 rounded-lg p-2 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-cyan-500 disabled:opacity-50" />
        <button type="submit" disabled={!chat || isLoading || !input.trim() || !!error} className="bg-cyan-500 text-white p-2 rounded-lg transition enabled:hover:bg-cyan-600 disabled:opacity-50 disabled:cursor-not-allowed" aria-label="Enviar mensagem"><PaperAirplaneIcon className="w-6 h-6" /></button>
      </form>
    </div>
  );
};
